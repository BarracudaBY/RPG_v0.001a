...
public class EnemyArcher : Enemy
{
	...
	[SerializeField] private GameObject arrowPrefub;
	[SerializeField] private float arrowSpeed;
	[SerializeField] private float arrowDamage;
	
	public Vector2 jumpVelocity;
	public float jumpcooldown;
	public float safeDiastance;                                        
	[HideInInspector] public float lastTimeJumped;
	
	[Header("AdditionaL collision check")]
	[SerializeField]private Transform grounBehindCheck;
   ...
   protected override void Awake()
   {
	...
	{
		GameObject newArrow = Instantiate(arrowPrefub, attackCheck.position, Quaternion.identity);
		
		newArrow.GetComponent<ArrowController>().SetupArrow(arrowSpeed * facingDir, stats);
		
	}
	
	public bool GroundBehind() => Physics2D.BoxCast(grounBehindCheck.position, groundBehindCheckSize, 0, Vector2.zero,0,   whatIsGround);
	...
	protected override void OnDrawGizmos()
	{
		...
		Gizmos.DrawWireCube(grounBehindCheck.position, groundBehindCheckSize);																				
